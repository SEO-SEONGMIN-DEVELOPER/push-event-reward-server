...
public class ConcertFacade {
    ...
    public String reserveWithKafka(Long concertId, Long memberId) {
        ...
        future.whenComplete((result, ex) -> {
            if (ex != null) {
                log.error("Kafka 메시지 전송 실패: requestId={}, concertId={}, memberId={}", requestId, concertId, memberId, ex);
                inventoryService.incrementSeat(concertId);
            } else {
                log.debug("Kafka 메시지 전송 성공: requestId={}, concertId={}, memberId={}, offset={}", 
                        requestId, concertId, memberId, result.getRecordMetadata().offset());
            }
        });
